...
-- ── Populate rider_profiles from existing users data (only if columns still exist) ──
DO $$
BEGIN
  IF EXISTS (
    ...
    SELECT id, vehicle_type, vehicle_plate, vehicle_reg_no, driving_license, vehicle_photo, documents, created_at, updated_at
    FROM users
    WHERE roles LIKE '%rider%'
      AND (vehicle_type IS NOT NULL OR vehicle_plate IS NOT NULL OR vehicle_reg_no IS NOT NULL
           OR driving_license IS NOT NULL OR vehicle_photo IS NOT NULL OR documents IS NOT NULL)
...
-- ── Populate vendor_profiles from existing users data (only if columns still exist) ──
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    SELECT id, store_name, store_category, store_banner, store_description, store_hours, store_announcement,
           COALESCE(store_min_order, 0), store_delivery_time, COALESCE(store_is_open, TRUE),
           store_address, business_type, business_name, ntn, created_at, updated_at
    FROM users
